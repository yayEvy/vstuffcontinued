...
public class RopeTextOverlay implements IGuiOverlay {
    ...
    public void render(ForgeGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        if (mc.options.hideGui || mc.gameMode.getPlayerMode() == GameType.SPECTATOR)
            return;
        if (RopeConnection.hoveringPos == null)
            return;
        if (RopeConnection.cached == null || !RopeConnection.cached.valid)
            return;
        if (RopeConnection.extraTipWarmup < 4)
            return;
        ...
        Color color = new Color(0x4ADB4A).setAlpha(Mth.clamp((RopeConnection.extraTipWarmup - 4) / 3f, 0.1f, 1));
