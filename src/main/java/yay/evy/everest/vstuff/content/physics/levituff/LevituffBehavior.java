...
public enum LevituffBehavior {
    UP_TO_Y_LEVEL((level, ship, levituffBlocks) -> {
        final double gravity = VStuffConfigs.server().gravity.get();
        ...
        levituffBlocks.forEach(pos -> {
            ...
            ship.applyModelForce(new Vector3d(0, spring + damping, 0), pos);
        });
    }),
    ...
    private final TriConsumer<PhysLevel, PhysShipImpl, Set<Vector3d>> onPhysTick;

    LevituffBehavior(TriConsumer<PhysLevel, PhysShipImpl, Set<Vector3d>> onPhysTick) {
